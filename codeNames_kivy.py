...
class codeNamesApp(App):
    ...
    def changeColorRed(self, instance):
        ...
        if self.red_cards == 0:
            self.bottom_label.text = '[b]Red team WON!!![/b]'
            ...
            self.bottom_label.color = (1,0,0,1)
            self.turn = 'RED'
    ...
    def changeColorBlue(self, instance):
        ...
        if self.blue_cards == 0:
            self.bottom_label.text = '[b]Blue team WON!!![/b]'
            for elements in self.gridLayout.children:
                elements.disabled = True
            self.startButton.disabled = False
            self.startButton.text = 'Play again?'
            ...
            self.bottom_label.color = (0,0,1,1)
            self.turn = 'BLUE'
    ...
    def changeColorYellow(self, instance):
        instance.background_normal = "Bystander_card2.jpg"
        instance.background_down = ''
        instance.color = (1,1,1,1)
        if self.turn == 'RED':
            self.turn = 'BLUE'
    ...
    def btn_create(self, instance):
        ...
        self.bottom_label.text = 'Red team\'s turn!'
        self.bottom_label.color = (1,0,0,1)
        self.btnCreate.markup = True
        self.btnDelete.markup = True
        self.btnCreate.text = 'Cards left for the [b]RED [/b] team: [b]' + str(self.red_cards) + "[/b]"
        self.btnDelete.text = 'Cards left for the [b]BLUE [/b] team: [b]' + str(self.blue_cards) + "[/b]"
        self.turn = 'RED'

        wordList = codeNames.loadWords()
        words = codeNames.selectWords(wordList)
        count = 0
        red_file = open("/Users/sophiahu/Desktop/RED_spymaster_words.txt", 'w')
        blue_file = open("/Users/sophiahu/Desktop/BLUE_spymaster_words.txt", 'w')
        for i in range(len(words)):
            if words[i].typeOfCard == 'RED':
                red_file.write(words[i].word + "\n")
            elif words[i].typeOfCard == 'BLUE':
                blue_file.write(words[i].word + "\n")
        red_file.close()
        blue_file.close()

        for row in range(ROWS):
            for col in range(COLS):
                button = Button(text=str(words[count].word), 
                                font_size=50,
    ...
    def build(self):
        ...
        self.btnCreate = RedLabel(text='Cards left for the [b]RED[/b] team: [b]' + str(self.red_cards) + "[/b]",
                                    font_size = 60,
                                    size_hint=(2, 1),
                                    markup = True)
        ...
        self.btnDelete = BlueLabel(text='Cards left for the [b]BLUE[/b] team: [b]' +  str(self.blue_cards) + "[/b]",
                                font_size = 60,
                                size_hint=(2, 1),
                                markup = True)
        ...
        self.bottom_label = BottomLabel(text='Welcome to [color=#FF0000]C[/color][color=#06038D]P[/color][color=#30B700]C[/color][color=#FFB300]B[/color] Codenames!!!', 
                                font_size=130, 
                                size_hint=(1,1),
                                color = (0,0,0,1),
                                markup = True)
